...
function Bolsa() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/produtos')
      .then((res) => {
        const bolsas = res.data.filter(p => p.nome.toLowerCase().includes('bolsa'));
        setProducts(bolsas);

        // Inicializa quantidade como 1 para cada bolsa
        const quantidadeInicial = {};
        bolsas.forEach(p => quantidadeInicial[p.id] = 1);
        ...
        {products.map(product => (
          <div key={product.id} className={Style.product_card}>
            <div className={Style.product_image} onClick={() => handleProductClick(product.foto)}>
              ...
              <span className={Style.cart_icon}><img src={CartIcon} alt="Adicionar ao carrinho" /></span>
            </div>
